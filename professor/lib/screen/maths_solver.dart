...
class MathsScreen extends StatefulWidget {
  ...
  @override
  State<MathsScreen> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<MathsScreen> {
  ...
  bool _loading = false;
  static const _apiKey = 'AIzaSyDyjSSHn8SjxUsDyz8M3Gy9Si4Qs6b8Vxg';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Gemini AI Professor'),
      ),
      body: Column(
...
class DataModel {
  final String image, type, prompt;
  final bool isMe;
  DataModel(this.image, this.type, this.prompt, {this.isMe = false});
}

class StreamSocket {
  final _socketResponse = StreamController<List<DataModel>>();

  void Function(List<DataModel>) get addResponse => _socketResponse.sink.add;

  Stream<List<DataModel>> get getResponse => _socketResponse.stream;
